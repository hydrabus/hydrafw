...
static void hydrabus_mode_read_error(t_hydra_console *con,
                                     uint32_t mode_status) {
  cprintf(con, mode_str_read_error, mode_status);
}

static void hydrabus_mode_write_error(t_hydra_console *con,
                                      uint32_t mode_status) {
  cprintf(con, mode_str_write_error, mode_status);
}

static void hydrabus_mode_write_read_error(t_hydra_console *con,
                                           uint32_t mode_status) {
...
#define PRINT_FREQ_1KHZ (1000)
  ...
  if (freq >= PRINT_FREQ_1GHZ) {
    ...
    freq_dec_part =
        (freq - (freq_int_part * PRINT_FREQ_1GHZ)) / (PRINT_FREQ_1MHZ * 10);
    suffix = "ghz";
  } else if (freq >= PRINT_FREQ_1MHZ) {
    freq_int_part /= PRINT_FREQ_1MHZ;
    freq_dec_part =
        (freq - (freq_int_part * PRINT_FREQ_1MHZ)) / (PRINT_FREQ_1KHZ * 10);
    suffix = "mhz";
  ...
  for (i = 0; i < count; i++) {
    if (p_proto->wwr == 1) {
      ...
      if (con->mode->exec->write_read != NULL) {
        mode_status = con->mode->exec->write_read(
            con, p_proto->buffer_tx, p_proto->buffer_rx, num_bytes);
      }
      ...
      if (con->mode->exec->write != NULL) {
        mode_status =
            con->mode->exec->write(con, p_proto->buffer_tx, num_bytes);
      }
      if (mode_status != HYDRABUS_MODE_STATUS_OK)
