// Original source https://github.com/encedo/hex2dfu/blob/master/hex2dfu.c
// MIT License (MIT)
// Modified by B.VERNOUX 2024
#include <ctype.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
unsigned char *ihex2bin_buf(unsigned int *start_address, unsigned int *dst_len, FILE *inFile, unsigned int max_address);
...
int main (int argc, char **argv) {
  int targen_number = 0;
  int c, vid =0x0483, pid = 0xdf11, ver = 0xffff;
  ...
  opterr = 0;
  while ((c = getopt (argc, argv, "hv:p:v:t:r:d:i:l:o:c:S:P:eJE:")) != -1) {
    switch (c) {
      case 'J':   
        json_output = 1;
        break;
...
void print_help(void) {
  printf("STM32 hex2dfu version 1.4.1\r\n");
...
unsigned char *ihex2bin_buf(unsigned int *start_address, unsigned int *dst_len, FILE *inFile, unsigned int max_address) {
  ...
  char oneline [512];
  unsigned char raw[256], start_set = 0, *dst = NULL;
